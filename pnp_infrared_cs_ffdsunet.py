...
def run():
    ...
    for t, ms in enumerate(test_loader):
        ...
        while i + block_size <= max_rol:
            ...
            while j + block_size <= max_col:
                ...
                for t in tqdm(range(30)):
                    ...
                    net_output = ffdnet_denosing(net_input, sigma, flag).clamp(0, 1)###############32,32,1
        ...
        cv2.imwrite("test_out/ffdsunet/chimney/orig_%.1f.bmp"% (CS_ratio) ,ms)
        Rec_ffdnet = Rec_ffdnet * 255.0
        cv2.imwrite("test_out/ffdsunet/chimney/Rec_FFDSUNet_%.1f.bmp"% (CS_ratio) ,Rec_ffdnet)
